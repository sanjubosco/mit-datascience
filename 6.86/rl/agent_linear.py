...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if (np.random.binomial(1,(1-epsilon))):
        action_index, object_index = np.unravel_index(np.argmax(np.dot(theta,state_vector.T)),(NUM_ACTIONS,NUM_OBJECTS))
    else:
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    if terminal:
        ...
        q_max = np.max(np.dot(theta,next_state_vector.T))
    
    q_value = np.dot(theta,current_state_vector.T)[tuple2index(action_index, object_index)]
    
    theta[tuple2index(action_index, object_index)] = theta[tuple2index(action_index, object_index)] + ALPHA*((reward + GAMMA*q_max) - q_value)*current_state_vector
